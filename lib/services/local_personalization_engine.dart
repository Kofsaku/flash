...
class LocalPersonalizationEngine {
  static final LocalPersonalizationEngine _instance =
  ...
  Future<List<Example>> generateMixedExamples(
    ...
    if (profile == null) {
      print(
        'LocalPersonalizationEngine: No profile provided, returning base examples only',
      );
      return baseExamples;
    }
    ...
    print(
      'LocalPersonalizationEngine: Generated ${personalizedExamples.length} personalized examples for category $categoryId',
    );
  ...
  List<Example> _generatePersonalizedExamples(
    ...
    try {
      ...
      print(
        'LocalPersonalizationEngine: Profile occupation: ${profile.occupation}',
      );
      ...
      print(
        'LocalPersonalizationEngine: Found ${relevantTemplates.length} templates for category $categoryId',
      );

      if (relevantTemplates.isEmpty) {
        print(
          'LocalPersonalizationEngine: No templates found for category $categoryId',
        );
        return examples;
      }
      ...
      for (ExampleTemplate template in relevantTemplates) {
        ...
        // プロファイルに基づいて変数を選択
        Map<String, String> selectedVariables = _selectVariables(
          template,
          profile,
        );

        print(
          'LocalPersonalizationEngine: Template ${template.id} - selected variables: $selectedVariables',
        );
        ...
        if (selectedVariables.isNotEmpty) {
          ...
          if (personalizedExample != null) {
            ...
            print(
              'LocalPersonalizationEngine: Failed to apply template ${template.id}',
            );
          }
        } else {
          print(
            'LocalPersonalizationEngine: No variables selected for template ${template.id}',
          );
        }
      }

      print(
        'LocalPersonalizationEngine: Generated $generatedCount personalized examples from ${relevantTemplates.length} templates',
      );
    } catch (e) {
      print(
        'LocalPersonalizationEngine: Error generating personalized examples: $e',
      );
    }
  ...
  Map<String, String>? _selectBestVariableValue(
    ...
    print(
      'LocalPersonalizationEngine: Variable options keys: ${variableOptions.keys}',
    );

    // 1. 職業ベースの選択を優先
    if (variableOptions.containsKey('occupation') &&
        profile.occupation != null) {
      print(
        'LocalPersonalizationEngine: Looking for occupation ${profile.occupation} in ${variableOptions['occupation']!.keys}',
      );
      var occupationOptions =
          variableOptions['occupation']![profile.occupation];
      if (occupationOptions != null &&
          occupationOptions is List &&
          occupationOptions.isNotEmpty) {
        List<dynamic> selected = _randomSelect(occupationOptions);
        if (selected.length >= 2) {
          print(
            'LocalPersonalizationEngine: Selected occupation-based value: ${selected[0]} → ${selected[1]}',
          );
    ...
    if (variableOptions.containsKey('hobby') && profile.hobbies.isNotEmpty) {
      for (String hobby in profile.hobbies) {
        ...
        if (hobbyOptions != null &&
            hobbyOptions is List &&
            hobbyOptions.isNotEmpty) {
          dynamic selected = _randomSelect(hobbyOptions);
          if (selected is List && selected.length >= 2) {
            print(
              'LocalPersonalizationEngine: Selected hobby-based value: ${selected[0]} → ${selected[1]}',
            );
            return {'japanese': selected[0], 'english': selected[1]};
          }
        }
    ...
    // 3. 家族構成ベースの選択
    if (variableOptions.containsKey('familyStructure') &&
        profile.familyStructure != null) {
      var familyOptions =
          variableOptions['familyStructure']![profile.familyStructure];
      if (familyOptions != null &&
          familyOptions is List &&
          familyOptions.isNotEmpty) {
        dynamic selected = _randomSelect(familyOptions);
        if (selected is List && selected.length >= 2) {
          print(
            'LocalPersonalizationEngine: Selected family-based value: ${selected[0]} → ${selected[1]}',
          );
          return {'japanese': selected[0], 'english': selected[1]};
        }
      }
    ...
    if (variableOptions.containsKey('general')) {
      ...
      if (generalOptions != null &&
          generalOptions is List &&
          generalOptions.isNotEmpty) {
        dynamic selected = _randomSelect(generalOptions);
        if (selected is List && selected.length >= 2) {
          print(
            'LocalPersonalizationEngine: Selected general value: ${selected[0]} → ${selected[1]}',
          );
          return {'japanese': selected[0], 'english': selected[1]};
        }
      }
  ...
  Example? _applyTemplate(
    ...
    try {
      ...
      if (japanese.contains('{') || english.contains('{')) {
        print(
          'LocalPersonalizationEngine: Template application failed, variables remain in text',
        );
        return null;
      }
      ...
      print(
        'LocalPersonalizationEngine: Created personalized example: $japanese → $english',
      );
  ...
  List<Example> _mixExamples(
    ...
    if (baseExamples.length >= 3 && personalizedExamples.isNotEmpty) {
      int insertInterval =
          (baseExamples.length / (personalizedExamples.length + 1)).floor();
    ...
    print(
      'LocalPersonalizationEngine: Mixed ${baseExamples.length} base examples with ${personalizedExamples.length} personalized examples',
    );
